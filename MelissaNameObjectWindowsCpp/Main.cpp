 ...
 *   mdNameEnums.h declare the API, mdName.lib is the import library the linker resolves
 *   against, and mdName.dll carries the implementation. The accompanying
 *   MelissaNameObjectWindowsCpp.ps1 script downloads all four on every run.
 *
 * Reference:
...
void RunAsConsole(string license, string testName, string dataPath)
{
	cout << "\n============ WELCOME TO MELISSA NAME OBJECT WINDOWS C++ ============\n" << endl;
	...
	while (shouldContinueRunning)
	{
		...
		if (testName.empty())
		{
			...
			strcpy_s(dataContainer.Name, input.c_str());
		}
		else
		{
			// One-shot mode: use the name passed on the command line.
			strcpy_s(dataContainer.Name, testName.c_str());
		}
